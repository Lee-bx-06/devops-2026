...
小组 A09 / 配对组 第 9 组（pair09） ｜ 契约版本 1.0.0 ｜ 负责人 A1（谢浩天），B1 复核
...
### B1 复核补充：`VALIDATION_2xxx` 的适用边界（2026-09-21）

第 9 页只给出三个系统执行错误码，且都写在 `job.error` 里。`VALIDATION_2xxx` 是本组新增的，
必须把它的适用范围限死，否则会和第 9 页「系统执行错误写入 `job.error`」的约定打架。

| 项 | 约定 |
| --- | --- |
| 出现时机 | 仅在创建期同步拒绝时出现，即请求体不符合 `task.schema.json`、幂等键冲突、端点与 `job_type` 不一致 |
| 出现位置 | HTTP 4xx 响应体的错误对象，**不写入 `job.error`** |
| 是否产生 Job | 不产生。没有 `job_id`，因此不可能出现在 Job 文档里 |
| 与 `job.error` 的关系 | 两套码段互斥。`job.error.code` 只取 `ENV_3xxx` / `EXEC_4xxx` / `ANALYSIS_5xxx` 三段 |
| 下游处理 | 调用方按 `code` 修正请求后重试，不重试原请求 |

采用「进注册表」而不是「只用裸 HTTP 4xx」的理由：调用方可以用同一套 `code` 判断失败归属，
不必区分「HTTP 状态码语义」与「业务码语义」两套逻辑。代价是注册表要同时管住两套码段，
因此本表新增了「出现位置」这一列的约束。
...
## 五、B1 复核结论（2026-09-22）

本节原为「待 B1 复核」清单。四条已于 2026-09-22 全部关闭，
上表八个码即注册表的当前版本。

| # | 待复核项 | B1 结论 | 理由与落点 |
| --- | --- | --- | --- |
| 1 | `VALIDATION_2xxx` 命名空间是否接受，或改用裸 HTTP 4xx | **接受，限定为创建期拒绝** | 只出现在 HTTP 4xx 响应体，永不写入 `job.error`；边界见第二节「B1 复核补充」。用同一套 `code` 表达失败归属，调用方不必区分 HTTP 语义与业务语义两套逻辑 |
| 2 | `ENV_3003`、`EXEC_4001`、`ANALYSIS_5002`、`VALIDATION_2002` 四个扩展码 | **四个全部接受** | `ENV_3003` 对应第 5 页「接收方检查基线版本和配置匹配」（A3 的 `echecker-contract.md` 第四节已落为运行期 `FAILED`）；`VALIDATION_2002` 是幂等键重放的必要区分；`EXEC_4001` 与 `ANALYSIS_5002` 分别补齐「执行器崩溃」与「产物写出失败」两个第 9 页未给码的阶段，与既有三段命名空间无重叠 |
| 3 | `retryable` 的建议值是否与 B 侧的重试策略一致 | **接受为契约默认值** | 确定性失败为 `false`（请求结构、幂等键冲突、基线不匹配、超时、分析器失败），可恢复失败为 `true`（镜像构建、工作进程被终止、产物写出）。**同一个 `code` 的 `retryable` 语义必须稳定**，不得时而 `true` 时而 `false`；服务实现可自行决定是否重试，但不能把重试决策转嫁给调用方猜测 |
| 4 | 注册表维护权归 A1 还是 B1 | **归 B1** | 错误码变更属版本与兼容性范畴，按 `../versioning.md` 第四节流程处理，并在 `../CHANGELOG.md` 留记录。1.0.0 条目「B1 复核结论」第 7 项即此结论 |

注册表维护权归 B1 之后，本页仍是唯一的错误码清单：
新增码先在本页登记，再改 `tools/validate.py` 的取值校验与样例，最后 `make check`。
本节四条结论见 `../CHANGELOG.md`「B1 版本裁定与复核（2026-09-22）」。
