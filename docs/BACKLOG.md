...
小组 A09 ｜ 配对组 B09 ｜ 契约版本 1.0.0 ｜ 本文件由 A1（谢浩天）维护
...
| # | 状态 | 事项 | 对方 | 阻塞了什么 |
| --- | --- | --- | --- | --- |
| 14 | ⬜ | `GET /v1/artifacts/{artifact_id}` 作为产物读取接口 | B1 | E12「证明另一组能读取文件」 |
| 15 | ⬜ | `VALIDATION_2xxx` 命名空间是否接受 | B1 | `errors.md` 注册表定稿 |
| 16 | ⬜ | 信封严格 / 载荷可扩展的分界线（ADR-004） | B1 | 四组能否自行扩展 `input`/`output` |
| 17 | ⬜ | `execution` 字段定义（第 19 页未解释，A1 自定） | B1 + 四组 | 若推翻属破坏性变更，需升版本 |
| 18 | ⬜ | URI 用完整 `job_id` 而非第 24 页简写 `full01` | B1 + 教师 | 有意偏离课件样例，需认可 |
...
| Issue + PR 关联 | ⬜ 未做 | 需在 GitHub 上开 | 提交后补 Issue 号与 PR 链接到 `CONTRIBUTIONS.md` |
