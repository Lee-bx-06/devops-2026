...
| 4 | ✅ | 产物访问约定 | A/B 共同 | `ADR-003` + `endpoints.md` 第四节 + `samples/artifact.error-report.json` | 说明下游读取方式 | URI 文法与 `GET /v1/artifacts/{artifact_id}` 已定义；B1 已接受读取端点 |

第 2、3 项标 ✅ 是指**契约层面**可表达且可校验。`baseline` 匹配语义由 A3
在 ADR-008 定稿，`MD` 的消费语义由 B3 在 ADR-009 定稿。
...
| 11 | 🟡 | ADR 汇总 | `docs/adr/ADR-001`–`ADR-011` | `ADR-007`–`ADR-009` 已接受；`ADR-010`、`ADR-011` 待相应评审 |
...
| 18 | ✅ | URI 用完整 `job_id` 而非第 24 页简写 `full01` | B1 已接受 | A09/B09 内部契约采用完整 `job_id`；课件 URI 仅作说明样例 |
...
第 18 项已由 B1 接受，A09/B09 的内部契约以完整 `job_id` 为准；第 24 页 URI 仅作说明样例。
...
| Issue + PR 关联 | ✅ 已完成 | 走 fork 流程（A1 对上游无推送权限） | Issue #1 追踪现存跨组确认项；PR #2 承载 A1 首轮交付物 |
...
| `configuration_id` 取值两组不一致 | ✅ 已定，🟡 A2 样例待裁定 | A2/B2 定为 `cc-gcc13-release-6f12a4c8`（不绑定 commit，符合 `$defs.configuration_id` 定义），A1 初版的 `cc-MODE0` 退役 | A1/A3/B2/B3 的样例已迁移；剩余见 `full-check.clean-project.json` 一行 |
...
| A2–B2 v0.1 交接清单 | ✅ 已完成 | canonical 样例、稳定配置 ID、独立命令、真实制品校验均已落地 | A2 已在 PR #6 复核并接受 ADR-007 |
