...
- 状态：**已接受**（A1 谢浩天，A09；`execution` 定义经 B1 复核）
- 日期：2026-09-21
- 契约版本：1.0.0
...
- 三种 `kind` 意味着样例数量翻倍：每类 job 至少要一对请求/响应。
  本仓库已备齐四类 job 的请求/响应正例，以及逐条覆盖上述规则的负例
  （清单与数量见 `../VALIDATION.md` 第五节，该处为唯一来源）。
- `execution` 由 A1 定义并经 B1 复核。若后续推翻，属第 26 页的
  破坏性变更，需升 `schema_version` 并同步四组样例。
